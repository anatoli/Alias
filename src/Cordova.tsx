...
interface ICordovaContext {
    ...
    platformId: string
    listen: (evtType: CordovaEventType, handler: any) => void
    unlisten: (evtType: CordovaEventType, handler: any) => void
}
...
const CordovaProvider: React.FC = React.memo(({ children }) => {
    ...
    const listen = (evtType: CordovaEventType, handler: any) => {
        document.addEventListener(evtType, handler)
    }

    const unlisten = (evtType: CordovaEventType, handler: any) => {
